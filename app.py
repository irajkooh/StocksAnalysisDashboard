...
# ─── 5. HF Space Keep-Alive Scheduler ────────────────────────────────────────
...
def start_keep_alive_scheduler(space_url: str):
    ...
    # Daily ping at 06:00 UTC — acts as a scheduled warm-up task
    scheduler.add_job(
        _ping_self,
    ...
    scheduler.start()
    logger.info(f"[keep-alive] scheduler started → pinging {space_url} every 30 min + daily at 06:00 UTC")
    return scheduler
...
def main():
    ...
    llm    = {"ollama": f"Ollama / {OLLAMA_MODEL}", "groq": "groq or llama", "hf": f"huggingface ({HF_MODEL})"}.get(LLM_PROVIDER, LLM_PROVIDER)
    ...
    # Step 6 — open browser in background (local only)
    if not IS_HF_SPACE:
    ...
    # Step 7 — launch Gradio in the main thread (required by Gradio 5)
    try:
        launch_frontend()
